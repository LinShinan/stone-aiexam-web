<template>
  <div class="admin-page">
    ...
    <div class="admin-action-card">
      <div class="admin-action-card-inner">
        <div class="admin-search-group">
          <el-input v-model="searchForm.keyword" placeholder="搜索视频标题或描述..." clearable @input="handleSearch" class="admin-search-input">
            <template #prefix><el-icon><Search /></el-icon></template>
          </el-input>
          <el-select v-model="searchForm.status" placeholder="选择状态" clearable @change="handleSearch" class="admin-filter-select">
            <el-option label="待审核" :value="0" />
            <el-option label="已发布" :value="1" />
            <el-option label="已拒绝" :value="2" />
            <el-option label="已下架" :value="3" />
          </el-select>
          <el-select v-model="searchForm.uploaderType" placeholder="上传者类型" clearable @change="handleSearch" class="admin-filter-select">
            <el-option label="用户投稿" :value="1" />
            <el-option label="管理员上传" :value="2" />
          </el-select>
          <el-select v-model="searchForm.categoryId" placeholder="选择分类" clearable @change="handleSearch" class="admin-filter-select">
      ...
      <el-table :data="videoList" v-loading="loading" stripe>
        ...
        <el-table-column label="封面" width="110">
          <template #default="{ row }">
            <el-image :src="row.coverUrl" :preview-src-list="[row.coverUrl]" fit="cover" class="admin-thumb-sm" :preview-teleported="true">
              <template #error>
                <div class="admin-thumb-placeholder"><el-icon><Picture /></el-icon></div>
              </template>
            </el-image>
          </template>
        </el-table-column>
        ...
        <el-table-column label="上传类型" width="90">
          <template #default="{ row }">
            <el-tag :type="row.uploaderType === 2 ? 'success' : 'info'" effect="plain">
              {{ row.uploaderType === 2 ? '管理员' : '用户投稿' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="viewCount" label="观看" width="75" align="center" />
        <el-table-column prop="likeCount" label="点赞" width="70" align="center" />
        <el-table-column prop="createdAt" label="创建时间" width="160" />
        <el-table-column label="操作" width="340" fixed="right">
          <template #default="{ row }">
            <div class="admin-action-links">
              <el-button size="small" type="primary" @click="previewVideo(row)">
                <el-icon><VideoPlay /></el-icon>
                <span>预览</span>
              </el-button>
              <el-button size="small" type="primary" @click="editVideo(row)">
                <el-icon><Edit /></el-icon>
                <span>编辑</span>
              </el-button>
              <el-button v-if="row.status === 0" size="small" type="success" @click="auditVideoAction(row, 1)">
                <el-icon><Check /></el-icon>
                <span>通过</span>
              </el-button>
              <el-button v-if="row.status === 0" size="small" type="warning" @click="showRejectDialog(row)">
                <el-icon><Close /></el-icon>
                <span>拒绝</span>
              </el-button>
              <el-button v-if="row.status === 1" size="small" type="warning" @click="offlineVideoAction(row)">
                <el-icon><Bottom /></el-icon>
                <span>下架</span>
              </el-button>
              <el-button size="small" type="danger" @click="deleteVideoAction(row)">
    ...
    <!-- 上传视频对话框 -->
    <el-dialog v-model="uploadDialogVisible" title="上传视频" width="780px" :close-on-click-modal="false">
      <el-form :model="uploadForm" :rules="uploadRules" ref="uploadFormRef" label-width="100px">
        ...
        <el-form-item label="视频分类" prop="categoryId">
          <el-cascader v-model="categoryPath" :options="categoryTree" :props="cascaderProps" placeholder="请选择分类" style="width: 100%" @change="handleCategoryChange" clearable />
        </el-form-item>
        <el-form-item label="视频标签" prop="tags">
          <div class="tags-input-wrap">
            ...
            <div class="tags-display" v-if="tagList.length > 0">
              <el-tag v-for="tag in tagList" :key="tag" closable @close="removeTag(tag)" class="tag-item">{{ tag }}</el-tag>
            </div>
          </div>
        </el-form-item>
        <el-form-item label="上传者名称" prop="uploaderName">
          <el-input v-model="uploadForm.uploaderName" placeholder="请输入上传者名称" />
        </el-form-item>
        <el-form-item label="视频时长" prop="duration">
          <div class="duration-wrap">
            <el-input-number v-model="uploadForm.duration" :min="1" placeholder="例如：300" />
            <span class="duration-unit">秒</span>
            <span class="duration-tip">请输入视频时长（秒），如5分钟=300秒</span>
          </div>
        </el-form-item>
        ...
        <el-form-item label="封面图片">
          <el-upload ref="coverUploadRef" :auto-upload="false" :limit="1" :on-change="handleCoverFileChange" accept="image/*" list-type="picture-card">
            <el-icon><Plus /></el-icon>
            <template #tip><div class="el-upload__tip">支持JPG、PNG格式，建议16:9，不超过2MB</div></template>
          </el-upload>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="previewDialogVisible" :title="currentPreviewVideo.title" width="900px">
      <div class="video-preview" v-if="currentPreviewVideo.fileUrl">
        <video :src="currentPreviewVideo.fileUrl" controls style="width: 100%; max-height: 500px; border-radius: 8px;" />
