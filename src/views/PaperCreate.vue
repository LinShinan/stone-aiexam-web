<template>
  <div class="paper-create-container">
    ...
    <el-tabs v-model="activeTab" class="creation-tabs">
      ...
      <!-- AI智能组卷 -->
      <el-tab-pane label="AI智能组卷" name="ai" :disabled="isEditMode">
        <el-form :model="aiForm" ref="aiFormRef" label-width="120px" class="ai-form">
          ...
          <div v-for="(config, index) in aiForm.configs" :key="index" class="ai-config-row">
            <div class="config-row-inner">
              <el-tag size="large" effect="dark" class="config-type-tag">{{ config.label }}</el-tag>
              <div class="config-fields">
                <div class="config-field">
                  <span class="config-field-label">题目分类</span>
                  <el-select
                    v-model="config.categoryIds"
                    placeholder="可多选，不选默认全部分类"
                    multiple
                    collapse-tags
                ...
                <div class="config-field config-field--narrow">
                  <span class="config-field-label">题目数量</span>
                  <el-input-number v-model="config.count" :min="0" controls-position="right" />
                </div>
                <div class="config-field config-field--narrow">
                  <span class="config-field-label">每题分值</span>
                  <el-input-number v-model="config.score" :min="0" controls-position="right" />
                </div>
              </div>
            </div>
          </div>
...
/* ===== 页面头部 ===== */
.paper-create-container :deep(.el-page-header) {
  background: linear-gradient(135deg, #fff 0%, #f0f9ff 100%);
  padding: 20px 24px;
  border-radius: 20px;
  box-shadow: 0 12px 32px -8px rgba(14, 165, 233, 0.12), 0 0 0 0.5px rgba(14, 165, 233, 0.1);
  position: relative;
  overflow: hidden;
}
.paper-create-container :deep(.el-page-header)::before {
  content: '';
  position: absolute;
...
.paper-create-container :deep(.el-page-header__left) {
  position: relative;
  z-index: 1;
}
.paper-create-container :deep(.el-page-header__content) {
  font-weight: 700;
  font-size: 18px;
  color: #0369a1;
}
...
.creation-tabs {
  ...
  box-shadow: 0 12px 40px -10px rgba(0,0,0,0.08), 0 0 0 0.5px rgba(0,0,0,0.05);
}
...
.creation-tabs :deep(.el-tabs__item):hover {
  color: #0ea5e9;
  background: #f0f9ff;
}
...
.question-table-container :deep(.el-table__header th) {
...
.question-table-container :deep(.el-table__row) {
  transition: all 0.2s ease;
}
.question-table-container :deep(.el-table__row:hover > td) {
  background: #f0f9ff;
}
.question-table-container :deep(.el-table__row) td {
...
/* 题目配置分隔线 */
.ai-form :deep(.el-divider) {
  margin: 28px 0 20px;
}
.ai-form :deep(.el-divider__text) {
  font-weight: 700;
  font-size: 15px;
  color: #0369a1;
  background: #fff;
...
.ai-config-row:hover {
  box-shadow: 0 8px 28px -8px rgba(14, 165, 233, 0.15);
  border-color: rgba(14, 165, 233, 0.25);
  transform: translateY(-1px);
}
.ai-config-row::before {
  content: '';
  position: absolute;
...
@media (max-width: 1200px) {
  .paper-create-container { max-width: 100%; }
  .ai-form { max-width: 100%; }
}

@media (max-width: 768px) {
  .filter-section .el-row { margin-bottom: 10px; }
  .filter-section .el-col { margin-bottom: 10px; }
  .summary-item { flex-direction: column; gap: 4px; }
  .summary-item .value { font-size: 20px; }
  .ai-config-row :deep(.el-row) { flex-direction: column; gap: 10px; }
  .ai-config-row :deep(.el-col) { max-width: 100% !important; flex: 1; }
}
</style> 